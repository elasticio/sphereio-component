 ...
 */
exports.addAttributes = function(schema, cfg, cb){
...
exports.readProduct = function (msg, cfg, cb){
    promiseProductTypeData(cfg).then(function(productTypeData){
        ...
        if (msg.body.masterVariant) {
            ...
            product.masterVariant.attributes = readAttributes(product.masterVariant.attributes, productTypeData);
        }

        if (msg.body.variants) {
            product.variants = msg.body.variants;
            for (var i = 0; i< product.variants.length; i++) {
                product.variants[i].attributes = readAttributes(product.variants[i].attributes, productTypeData);
            }
        }
 ...
 */
function readAttributes(attributes, productTypeData){
    var results = [];
    if (productTypeData && productTypeData.body && productTypeData.body.attributes) {
        _.each(productTypeData.body.attributes, function(attributeDescription){
            var value = readAttribute(attributeDescription, attributes);
            if (value !== undefined) {
                results.push({name: attributeDescription.name, value: value});
            }
        });
    }
    return results;
