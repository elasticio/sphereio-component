...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    function prerocessOrders(results) {
        ...
        _.each(results, addShippingPrice);

        function addShippingPrice(order){
            if (!order.shippingInfo || !order.shippingInfo.price) return;

            var shippingPrice = _.clone(order.shippingInfo.price);
            checkFreeShipping(shippingPrice, order.shippingInfo.shippingRate);
            addTaxRate(shippingPrice, order.shippingInfo.taxRate);

            order.shippingPrice = shippingPrice;
        }

        function checkFreeShipping(shippingPrice, shippingRate){
            if (!shippingRate || !shippingRate.freeAbove) return;
            checkSameCurrency(shippingPrice.currencyCode, shippingRate.freeAbove.currencyCode);
            if (shippingPrice.centAmount > shippingRate.freeAbove.centAmount) {
                shippingPrice.centAmount = 0;
            }
        }

        function addTaxRate(shippingPrice, taxRate){
            if (!taxRate || !taxRate.amount || taxRate.includedInPrice) return;
            shippingPrice.centAmount += Math.round(shippingPrice.centAmount * taxRate.amount);
        }
