...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    function convertSyncInfoIntoObject(ordersResponse) {
        _.each(ordersResponse.body.results, function preProcessOrder(order) {
            if (order.syncInfo.length > 1) throw new Error('SyncInfo should contain single element');
            order.syncInfo = order.syncInfo[0] || {};
        });
    ...
    queryOrders()
        ...
        .then(convertSyncInfoIntoObject)
