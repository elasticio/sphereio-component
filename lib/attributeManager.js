...
exports.addProductTypeData = function(schema, cfg, cb){
    exports.promiseProductTypeData(cfg).then(function(productTypeData){
        if (productTypeData && productTypeData.body.attributes) {
            schema.in.properties.masterVariant.properties.attributes = {
                "type": "object",
                "required": false,
                "properties" : exports.buildAttributes(productTypeData.body.attributes)
            };
        }
        cb(schema);
    });
 ...
 */
exports.buildAttributes = function (attributes) {
    var result = {};
    for (var i = 0; i < attributes.length; i++) {
        result[attributes[i].name] = exports.buildAttributeMeta(attributes[i]);
    }
    return result;
};
