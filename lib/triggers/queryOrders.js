...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    var reboundedOrderIds;
    var where;

    var snapshot = snapshot || {};
    snapshot.reboundedOrders = snapshot.reboundedOrders || {};
    reboundedOrderIds = _.keys(snapshot.reboundedOrders);

    if (reboundedOrderIds.length > 0) {
        where = '(lastModifiedAt > "' + lastModifiedAt + '" OR id in ("' + reboundedOrderIds.join('","') + '"))';
    } else {
