...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    snapshot.nonSyncedOrders = snapshot.nonSyncedOrders || {};
    ...
    function queryNonSyncedOrders(orderIds) {
    ...
    function addNonSyncedOrders(ordersResponse) {

        if (_.isEmpty(snapshot.nonSyncedOrders)) {
            return ordersResponse;
        }

        var orderIds = _.keys(snapshot.nonSyncedOrders);
        // skip orderId-s which are in ordersResponse already
        orderIds = _.reject(orderIds, function isRetreived(orderId) {
        ...
        return queryNonSyncedOrders(orderIds)
            .then(addNonSyncedOrdersToResponse)
            .then(returnOrdersResponse);

        function addNonSyncedOrdersToResponse(nonSyncedOrdersResponse) {
            if (!nonSyncedOrdersResponse) return;
            if (!nonSyncedOrdersResponse.body) return;
            _.each(nonSyncedOrdersResponse.body.results, function (order) {
    ...
    function checkUnsyncedCustomers(ordersResponse) {
        ...
        function checkForReject(order, key) {
            if (cfg.syncedCustomersOnly && order.customer && !order.customer.externalId) {
                snapshot.nonSyncedOrders[order.id] = true;
                delete ordersResponse.body.results[key];
                return true;
            } else {
                // passed
                delete snapshot.nonSyncedOrders[order.id];
                return false;
            }
        }
