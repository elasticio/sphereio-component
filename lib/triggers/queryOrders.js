...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    function addUnsyncedOrders(response) {
        ...
        function getIds(){
            ...
            console.log('See %s unsynced orders in snapshot', unsyncedOrderIds.length);
        ...
        function queryUnsyncedOrders(orderIds) {
            console.log('Get %s unsynced orders', Math.min(orderIds.length, UNSYNCED_LIMIT));
        ...
        function logOrdersCount(response, type){
            if (response && response.body && response.body.results) {
                console.log('Found %s %s orders', response.body.results.length, type);
            } else {
                console.log('Found 0 %s orders', response.body.results.length, type);
            }
        }
    ...
    function addCustomersData(response){
        ...
        console.log("join: " + 'id in ("' + customerIds.join('","') + '")');
        ...
        function getCustomerIds(orders){
            console.log("getCustomerIds: " + JSON.stringify(_.uniq(_.compact(_.pluck(orders, 'customerId')))));
            return _.uniq(_.compact(_.pluck(orders, 'customerId')));
        }

        function buildCustomersHash(customersResponse) {
            console.log("buildCustomersHash: " + JSON.stringify(_.indexBy(customersResponse.body.results, 'id')));
    ...
    function addPaymentData(response) {
        ...
        function getPaymentIDs(orders) {
            ...
            return _.uniq(paymentIds.map(function (element) {
                return element.payments[0].id;
            }));
        }

        function buildPaymentsHash(paymentsResponse){
            console.log("PaymentHash: " + JSON.stringify( _.indexBy(paymentsResponse.body.results, 'id')));
    ...
    function checkUnsyncedCustomers(response) {
        ...
        console.log('Remain unsynced %s orders', _.keys(snapshot.unsyncedOrders).length);
        return response;
