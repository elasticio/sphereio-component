...
function getMetaModel(cfg, callback) {
    ...
    function constructMetaData(err, schema) {
        ...
        if (!cfg.expandCustomers) {
            schema.properties.results.properties.customer = undefined;
        }
...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    function addCustomers(ordersResponse){
        if (!cfg.expandCustomers) return ordersResponse;
        ...
        function processCustomers(customersResponse){
            return addCustomersToOrders(ordersResponse, customersResponse);
        }
    }
