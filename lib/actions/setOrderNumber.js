...
function action(msg, cfg) {
    ...
    function setOrderNumber(response) {
        ...
        self.logger.info('Executing setOrderNumber with %s order number for order: %s', orderNumber, orderId);
    ...
    function handleError(err) {
        if (err.body && err.body.statusCode === 409) {
            ...
            self.logger.error('Error with set Order Number to sphere.io');
            console.dir(err);
            self.emit('error', err);
        }
    }
