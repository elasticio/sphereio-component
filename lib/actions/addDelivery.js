...
function action(msg, cfg) {
    ...
    function addDelivery(response) {
        self.logger.info('About to execute addDelivery action');
        ...
        self.logger.info('Executing addDelivery action with %s lineItemId for order: %s', lineItemId, orderId);
    ...
    function handleResult(response) {
        self.logger.info('Received response, emitting data');
    ...
    function handleError(err) {
        if (err.body && err.body.statusCode === 409) {
            ...
            self.logger.error('Error while processing addDelivery action in sphere.io component');
            console.dir(err);
            self.emit('error', err);
        }
    }
