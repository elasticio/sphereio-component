...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    function handleResults(ordersResponse) {
        if (ordersResponse.body.count) {
            prerocessOrders(ordersResponse.body.results);
    ...
    function prerocessOrders(results){
        _.each(results, function(order) {
            if (order.shippingInfo) {
                if (order.shippingInfo.shippingRate){
                    if (order.shippingInfo.shippingRate.freeAbove) {
                        if (order.shippingInfo.shippingRate.freeAbove.currencyCode !== order.shippingInfo.price.currencyCode) {
                            var msg = 'shippingRate.freeAbove.currencyCode %s is not equal to shippingInfo.price.currencyCode %s';
                            throw new Error(util.format(msg,order.shippingInfo.shippingRate.freeAbove.currencyCode,
                                order.shippingInfo.price.currencyCode));
                        } else {
                            if (order.shippingInfo.price.centAmount > order.shippingInfo.shippingRate.freeAbove.centAmount) {
                                order.shippingInfo.price.centAmount = 0;
                            }
                        }
                    }
                }
                if (order.shippingInfo.taxRate){
                    if (order.shippingInfo.taxRate.amount){
                        if (!order.shippingInfo.taxRate.includedInPrice) {
                            order.shippingInfo.price.centAmount += Math.round(order.shippingInfo.price.centAmount * order.shippingInfo.taxRate.amount);
                        }
                    }
                }
            }
