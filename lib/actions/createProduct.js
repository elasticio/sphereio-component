...
function getMetaModel(cfg, callback) {
    ...
    function constructMetaData(err, schema) {
        ...
        attributeManager.addAttributes(schema, cfg, function(){
...
function process(msg, cfg) {
    ...
    attributeManager.readProduct(msg, cfg, function(err, product) {
        if (err) {
            return hanldeError(err);
        }

        var client = sphere.createClient(cfg);
        client.products.save(product).then(handleResult).fail(hanldeError).done(end);
