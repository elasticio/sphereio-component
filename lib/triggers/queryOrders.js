...
function processTrigger(msg, cfg, next, snapshot) {
    ...
    // updates response.body.results with 'payment' property
    function addPaymentData(response) {
        ...
        function getPaymentIDs(orders) {
            var paymentIds = _.uniq(_.compact(_.pluck(orders, 'paymentInfo')));

            return paymentIds.map(function (element) {
        ...
        function addPaymentsToOrders(paymentsHash) {
            _.each(response.body.results, function assignOrderCustomer(order) {
                order.payment = paymentsHash[order.customerId];
            });
        }
