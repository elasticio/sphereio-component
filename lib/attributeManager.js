...
exports.addOptionalAttributes = function(schema, cfg, cb){
    promiseProductTypeData(cfg).then(function(productTypeData){
        if (productTypeData && productTypeData.body.attributes) {
            schema.in.properties.masterVariant.properties.attributes = describeAttributes(productTypeData.body.attributes);
        }
        cb(schema);
    });
};
...
function describeAttributes(attributes) {
    ...
    for (var i = 0; i < attributes.length; i++) {
        result[attributes[i].name] = describeAttribute(attributes[i]);
    }
    return result;
}
